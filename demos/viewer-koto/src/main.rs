...
fn koto_script_thread(
    rx: Receiver<String>,
    tx: Sender<Result<fidget::koto::ScriptContext, String>>,
) -> Result<()> {
    let mut engine = fidget::koto::Engine::new();
    loop {
        let script = rx.recv()?;
...
fn render_thread<F>(
    ...
    rx: Receiver<Result<fidget::koto::ScriptContext, String>>,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    std::thread::spawn(move || {
        let _ = file_watcher_thread(&path, file_watcher_rx, koto_script_tx);
        info!("file watcher thread is done");
    });
    std::thread::spawn(move || {
        let _ = koto_script_thread(koto_script_rx, koto_result_tx);
    ...
    std::thread::spawn(move || {
        ...
        let _ =
            render_thread::<F>(config_rx, koto_result_rx, render_tx, wake_tx);
